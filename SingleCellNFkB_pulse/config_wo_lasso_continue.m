function [cfg] = config_wo_lasso_continue(jobname,trajectory_index)
...
LB = -5;
UB = 10;
%bounds on regularization parameters are set to match config_lasso_continue even though there is only a uniform prior here
LB1 = -35; 
UB1 = 6;
cfg.param_defs = { 
...
struct('name','A20_par', 'prior','uniform','min',LB1,'max',UB1, 'units','uM'), ...%A20 mediated TNFR inactivation
struct('name','Activation_par', 'prior','uniform','min',LB1,'max',UB1, 'units','uM'), ...%A20 mediated TNFR inactivation
struct('name','ikb_par', 'prior','uniform','min',LB1,'max',UB1, 'units','uM'), ...%A20 mediated TNFR inactivation
};
%%
...
cfg.nswaps = 990000;                    % number of chain swaps (=number of saved samples!)
...
cfg.adapt_last = 10000;                 % last adaption step
cfg.energy_init_max = 1e8;%10000;             % maximinm allowed energy for initialization
...
for d = 1 : cfg.nexpt
    ...
    cfg.data{d}.time = expt{trajectory_index}.time;
    cfg.data{d}.mean = expt{trajectory_index}.mean;
    cfg.data{d}.stdev = 0.1*expt{trajectory_index}.mean;
    cfg.data{d}.nsamples = expt{trajectory_index}.nsamples;
    cfg.data{d}.weight   = expt{trajectory_index}.weight;
end
