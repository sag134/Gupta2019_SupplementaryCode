%Code for Table S2,S3 NFkB without lasso
...
for trajectory_number = [2,3,4]
    ...
    par_matrix2 = zeros(max_num_pts,dim,num_repeats);
    ...
    for i = 1:2
        ...
        for repeat = i:2:num_repeats
            ...
            params = data.params_chain(1,1:dim,LB:UB);
            num_pts = length(LB:UB);
            tmp{repeat} = reshape(params,dim,num_pts); 
            par_matrix2(:,:,repeat) = tmp{repeat}';
   ...
   % save(['nfkb_wolasso_trajectory_',num2str(trajectory_number)],'m','m1','LB','UB');
end
