/*   
**   perfect_adaptation_cvode.c
**	 
**   Cvode-Mex implementation of BioNetGen model 'perfect_adaptation'.
**
...
**          -lsundials_nvecserial -lsundials_cvode -lm perfect_adaptation_cvode.c
**
...
**        = perfect_adaptation_cvode( timepoints, species_init, parameters )
**
**   timepoints      : column vector of time points returned by integrator.
**   parameters      : row vector of 7 parameters.
...
/* Problem Dimensions */
#define __N_PARAMETERS__   7
#define __N_EXPRESSIONS__  13
...
calc_expressions ( N_Vector expressions, double * parameters )
{
    ...
    NV_Ith_S(expressions,7) = pow(10.0,NV_Ith_S(expressions,0));
    NV_Ith_S(expressions,8) = pow(10.0,NV_Ith_S(expressions,1));
    NV_Ith_S(expressions,9) = pow(10.0,NV_Ith_S(expressions,2));
    NV_Ith_S(expressions,10) = pow(10.0,NV_Ith_S(expressions,3));
    NV_Ith_S(expressions,11) = pow(10.0,NV_Ith_S(expressions,4));
    NV_Ith_S(expressions,12) = pow(10.0,NV_Ith_S(expressions,5));
...
{  
    NV_Ith_S(ratelaws,0) = NV_Ith_S(expressions,7)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,1) = NV_Ith_S(expressions,8)*NV_Ith_S(species,2)*NV_Ith_S(species,1);
    NV_Ith_S(ratelaws,2) = NV_Ith_S(expressions,9)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,3) = NV_Ith_S(expressions,10)*NV_Ith_S(species,2);
    NV_Ith_S(ratelaws,4) = NV_Ith_S(expressions,11)*NV_Ith_S(species,1);
    NV_Ith_S(ratelaws,5) = NV_Ith_S(expressions,12);
...
calc_species_deriv ( realtype time, N_Vector species, N_Vector Dspecies, void * f_data )
{
    ...
    NV_Ith_S(Dspecies,0) = 0.0;
    NV_Ith_S(Dspecies,1) = NV_Ith_S(ratelaws,0) -NV_Ith_S(ratelaws,1) -NV_Ith_S(ratelaws,4) +NV_Ith_S(ratelaws,5);
    NV_Ith_S(Dspecies,2) = NV_Ith_S(ratelaws,2) -NV_Ith_S(ratelaws,3);
...
void mexFunction( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] )
{
    ...
    {  mexErrMsgTxt("PARAMS must be a column vector with 7 elements.");  }
