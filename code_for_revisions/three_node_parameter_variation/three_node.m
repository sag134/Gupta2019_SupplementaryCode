function [err, timepoints, species_out, observables_out ] = three_node( timepoints, species_init, parameters, suppress_plot )
%THREE_NODE Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'three_node' and then (optionally) plots the observable trajectories,
...
%   source in file three_node_cvode.c must be compiled (see that file for details).
%   THREE_NODE returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = three_node( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
...
param_labels = { 'k1', 'k2', 'k3', 'k4', 'k5', 'k6' };
...
try 
    % run simulation
    [err, species_out, observables_out] = three_node_cvode( timepoints, species_init, parameters );
catch
...
if ( ~suppress_plot )
    ...
    title('three_node observables','fontSize',14,'Interpreter','none');
