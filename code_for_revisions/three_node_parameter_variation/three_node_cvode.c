...
**   three_node_cvode.c
**	 
**   Cvode-Mex implementation of BioNetGen model 'three_node'.
**
...
**          -lsundials_nvecserial -lsundials_cvode -lm three_node_cvode.c
**
...
**        = three_node_cvode( timepoints, species_init, parameters )
**
**   timepoints      : column vector of time points returned by integrator.
**   parameters      : row vector of 6 parameters.
...
/* Problem Dimensions */
#define __N_PARAMETERS__   6
#define __N_EXPRESSIONS__  12
#define __N_OBSERVABLES__  1
#define __N_RATELAWS__     6
...
calc_expressions ( N_Vector expressions, double * parameters )
{
    ...
    NV_Ith_S(expressions,6) = pow(10.0,NV_Ith_S(expressions,0));
    NV_Ith_S(expressions,7) = pow(10.0,NV_Ith_S(expressions,1));
    NV_Ith_S(expressions,8) = pow(10.0,NV_Ith_S(expressions,2));
    NV_Ith_S(expressions,9) = pow(10.0,NV_Ith_S(expressions,3));
    NV_Ith_S(expressions,10) = pow(10.0,NV_Ith_S(expressions,4));
    NV_Ith_S(expressions,11) = pow(10.0,NV_Ith_S(expressions,5));
...
{  
    NV_Ith_S(ratelaws,0) = NV_Ith_S(expressions,6)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,1) = NV_Ith_S(expressions,11)*NV_Ith_S(species,0);
    NV_Ith_S(ratelaws,2) = NV_Ith_S(expressions,7)*NV_Ith_S(species,1);
    NV_Ith_S(ratelaws,3) = NV_Ith_S(expressions,8)*NV_Ith_S(species,1);
    NV_Ith_S(ratelaws,4) = NV_Ith_S(expressions,9)*NV_Ith_S(species,2);
    NV_Ith_S(ratelaws,5) = NV_Ith_S(expressions,10)*NV_Ith_S(species,2);
...
calc_species_deriv ( realtype time, N_Vector species, N_Vector Dspecies, void * f_data )
{
    ...
    NV_Ith_S(Dspecies,0) = -NV_Ith_S(ratelaws,0) -NV_Ith_S(ratelaws,1) +NV_Ith_S(ratelaws,2) +NV_Ith_S(ratelaws,5);
    NV_Ith_S(Dspecies,1) = NV_Ith_S(ratelaws,0) -NV_Ith_S(ratelaws,2) -NV_Ith_S(ratelaws,3) +NV_Ith_S(ratelaws,4);
    NV_Ith_S(Dspecies,2) = NV_Ith_S(ratelaws,1) +NV_Ith_S(ratelaws,3) -NV_Ith_S(ratelaws,4) -NV_Ith_S(ratelaws,5);
...
void mexFunction( int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[] )
{
    ...
    {  mexErrMsgTxt("SPECIES_INIT must be a row vector with 3 elements.");  } 

    /* make sure params has correct dimensions */
    if ( (mxGetM(prhs[2]) != 1)  ||  (mxGetN(prhs[2]) != __N_PARAMETERS__) )
    {  mexErrMsgTxt("PARAMS must be a column vector with 6 elements.");  }
