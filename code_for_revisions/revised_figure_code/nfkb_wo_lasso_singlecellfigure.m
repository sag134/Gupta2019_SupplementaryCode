...
addpath('/shared3/LabUserFiles/Sanjana_Gupta/shared_2/LassoManuscript/SingleCellNFkB_pulse/')
%%
step = 2e4; % step interval at which to simulate single cell trajectories
f = figure('units','normalized','outerposition',[0,0,1,0.5]);
c = 1;
repeat_index = 1:2:12;
for trajectory_number = [2,3,4]%,3,4] % these are the trajectories for which the fits converged   
    mu = -25;
    b = 2;
...
%toPDF(f,'../RevisedFiguresMATLAB/nfkb_wo_group_lasso_fit_figure_Nov28.pdf');




%toPDF(f,'../RevisedFiguresMATLAB/nfkb_wo_group_lasso_fit_figure_Dec8.pdf');
