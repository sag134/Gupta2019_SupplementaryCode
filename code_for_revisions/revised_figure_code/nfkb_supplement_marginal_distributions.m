...
L_par = log10([1e5,2e5,7000,1.2e-5,1.2e-3,nan,nan,5e-7,nan,1e-2,nan,2e-3,5e-3,5e-2,1e-4,2e-5,nan,nan]); %For basal ikb degradation we used the unphosphorylated rates since IKK was not involved
...
%removing IKK because its modeled as a step

W_par = log10([0.0875/conv,nan,nan,nan,nan,nan,nan,30 * conv / 60, 0.00006 / 60,5.4/60,0.0048/60,0.018/60,0.012/60,0.828/60,0.12/60,0.00006/60,nan,nan,nan]);


clr_fr = [0.5,1,1];
tcounter=0;
repeat_index = 1:2:12;
for trajectory_number = 2%:4 % showing results for a representative trajectory
    tcounter=tcounter+1;
    LB = 5e4;
    UB = 9.9e5;
    ...
    for repeat = 1:num_repeats
        ...
        data = load(['/shared3/LabUserFiles/Sanjana_Gupta/shared_2/LassoManuscript/SingleCellNFkB_pulse/SingleCellNFkB_continue_chain_reduced_model_fixedstart_withlasso_mu_',num2str(mu),'_b_',num2str(b),'_trajectory_',num2str(trajectory_number),'_repeat_',num2str(repeat_index(repeat)),'_progress990000.mat']);    
        e = data.energy_chain(1,:);  
        % From the first repeat get a representative best-fit parameter set
        if min(e)<emin %& repeat==1
            ind = find(e ==min(e),1);
            emin = e(ind);
            par_min = data.params_chain(1,:,ind);
            save(['best_params_lasso_',num2str(trajectory_number)],'repeat','ind','emin','par_min');
        end     
    ...
    for i = 1:3
        axes(ha(i));
        p = par_matrix(:,i,:);
        h = histogram(p(:),1000,'EdgeColor','none','FaceAlpha',0.5,'FaceColor',[0.8,0.8,0.8]*clr_fr(tcounter)); hold on
        ...
        if i <= length(W_par) &&  ~isnan(W_par(i)) 
            xx = [W_par(i),W_par(i)];
            hold on;
    ...
    %save(['../RevisedFiguresMATLAB/Dec8_Lasso_NFkB_supplement_posterior_distribution',num2str(trajectory_number),'.mat'],'f2')
    %toPDF(f2,['../RevisedFiguresMATLAB/Dec8_Lasso_NFkB_supplement_posterior_distribution',num2str(trajectory_number),'.pdf'])
end
%%
