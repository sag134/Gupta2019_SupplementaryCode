function [err, timepoints, species_out, observables_out ] = five_node( timepoints, species_init, parameters, suppress_plot )
%FIVE_NODE Integrate reaction network and plot observables.
%   Integrates the reaction network corresponding to the BioNetGen model
%   'five_node' and then (optionally) plots the observable trajectories,
...
%   source in file five_node_cvode.c must be compiled (see that file for details).
%   FIVE_NODE returns an error value, a vector of timepoints,
%   species trajectories, and observable trajectories.
%   
%   [err, timepoints, species_out, observables_out]
%        = five_node( timepoints, species_init, parameters, suppress_plot )
%
%   INPUTS:
%   -------
%   timepoints      : column vector of time points returned by integrator.
%   species_init    : row vector of 5 initial species populations.
%   parameters      : row vector of 20 model parameters.
...
if ( isempty(parameters) )
   parameters = [ 0, -20, -20, -20, -20, -20, -1, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20, -20 ];
end
% check that parameters has proper dimensions
if (  size(parameters,1) ~= 1  ||  size(parameters,2) ~= 20  )
    fprintf( 1, 'Error: size of parameter argument is invalid! Correct size = [1 20].\n' );
    err = 1;
    return;
end
...
param_labels = { 'k1', 'k2', 'k3', 'k4', 'k5', 'k6', 'k7', 'k8', 'k9', 'k10', 'k11', 'k12', 'k13', 'k14', 'k15', 'k16', 'k17', 'k18', 'k19', 'k20' };
...
try 
    % run simulation
    [err, species_out, observables_out] = five_node_cvode( timepoints, species_init, parameters );
catch
...
if ( ~suppress_plot )
    ...
    title('five_node observables','fontSize',14,'Interpreter','none');
